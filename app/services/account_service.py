...
class AccountService:
    ...
    async def upsert_account(account: UnifiedAccount) -> UnifiedAccount:
        ...
        try:
            ...
            # Prepare complete account data for upsert
            # This will be used for INSERT or UPDATE
            account_data = {
                "user_id": str(account.user_id),
                "connection_id": account.connection_id,
                "external_account_id": account.external_account_id,
            ...
            # Only set created_at for new records (not on conflict/update)
            # Supabase upsert uses ON CONFLICT and won't override created_at on update
            ...
            # onConflict specifies the unique constraint column (external_account_id)
            result = await db.table("accounts").upsert(
                account_data,
                on_conflict="external_account_id"
            ).execute()

            if result.data:
                logger.info(
                    f"Account upserted [{hash_account_id(account.external_account_id)}] "
