...
class AccountService:
    ...
    async def create_account(account: UnifiedAccount) -> UnifiedAccount:
        ...
        try:
            ...
            result = db.table("accounts").insert(account_data).execute()
    ...
    async def get_account(account_id: int) -> Optional[UnifiedAccount]:
        ...
        try:
            db = get_db()
            result = db.table("accounts").select("*").eq("account_id", account_id).execute()
    ...
    async def get_account_by_external_id(external_account_id: str) -> Optional[UnifiedAccount]:
        ...
        try:
            db = get_db()
            result = db.table("accounts").select("*").eq("external_account_id", external_account_id).execute()
    ...
    async def get_user_accounts(user_id: UUID) -> List[UnifiedAccount]:
        ...
        try:
            db = get_db()
            result = db.table("accounts").select("*").eq("user_id", str(user_id)).execute()
    ...
    async def get_accounts_by_connection(connection_id: int) -> List[UnifiedAccount]:
        ...
        try:
            db = get_db()
            result = db.table("accounts").select("*").eq("connection_id", connection_id).execute()
            return [UnifiedAccount(**acc) for acc in result.data]
    ...
    async def update_account(account_id: int, updates: dict) -> UnifiedAccount:
        ...
        try:
            ...
            result = db.table("accounts").update(updates).eq("account_id", account_id).execute()

            if result.data:
                logger.info(f"Account updated [{hash_account_id(account_id)}]")
                return UnifiedAccount(**result.data[0])
    ...
    async def delete_account(account_id: int) -> bool:
        ...
        try:
            db = get_db()
            db.table("accounts").delete().eq("account_id", account_id).execute()
            logger.info(f"Account deleted [{hash_account_id(account_id)}]")
            return True
